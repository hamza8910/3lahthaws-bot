...
const statusMessages = {
    alreadyConfiguring: 'اسمحلنا اللعبة بدات نحي صبعك من تماك تعيش 😅',
    alreadyStarted: 'اللعبة شغالة! انتظروا تخلص واي لعبة جديدة 🎮',
    typeSelected: (type) => `✅ تم اختيار ${type}! ننتقل للإعدادات...`,
    configTimeout: '⏰ انتهت مهلة الإعداد. ابدأوا لعبة جديدة بـ /newgame'
};
...
// دالة عرض حالة اللعبة
function getGameStatusMessage(game) {
    switch (game.status) {
        case 'joining':
            return 'مرحلة الانضمام جارية';
        case 'configuring':
            return 'جاري إعداد اللعبة';
        case 'distributing':
            return 'جاري توزيع الأدوار';
        case 'playing':
            return 'اللعبة جارية';
        case 'voting':
...
– فريق جواسيس مش عارفينه`;
        ...
        // معالجة بدء اللعبة
            ...
            const joinKeyboard = {
    inline_keyboard: [
        [{ text: '🟢 انضم للعبة', url: `https://t.me/spy_spy_bbot?start=join${gameId}` }],
...
// معالجة اختيار نوع اللعبة
    ...
    // فحص إذا كانت اللعبة بدأت الإعداد بالفعل
    if (game_type.status === 'configuring' || game_type.status === 'distributing' || game_type.status === 'playing') {
        return bot.answerCallbackQuery(callbackQuery.id, { 
            text: 'اسمحلنا اللعبة بدات نحي صبعك من تماك تعيش',
            show_alert: true 
        });
    }

    // فحص إذا كان نوع اللعبة محدد بالفعل
    ...
    // تحديد نوع اللعبة والانتقال للإعداد
    game_type.gameType = gameType;
    game_type.status = 'configuring';
    game_type.configuredBy = userId; // تسجيل من بدأ الإعداد
    game_type.configStartTime = Date.now();
    
    const gameTypeName = gameType === 'items' ? 'الأشياء' : 'الأماكن';
    
    await bot.sendMessage(chatId, `✅ تم اختيار ${gameTypeName}\n📝 كم عدد الأشخاص العاديين؟ (3-30)`);
    
    data.games[gameId_type].waitingFor = 'normalPlayers';
    saveData();

// مؤقت انتهاء الإعداد (5 دقائق) - كود صحيح
setTimeout(async () => {  // ✅ أضف async هنا
    if (data.games[gameId_type] && data.games[gameId_type].status === 'configuring' && data.games[gameId_type].waitingFor) {
        await bot.sendMessage(chatId, '⏰ انتهت مهلة الإعداد. يمكنكم بدء لعبة جديدة.');
        delete data.games[gameId_type];
        saveData();
    }
}, 5 * 60 * 1000); // 5 دقائق

    // إشعار بنجاح الاختيار
    bot.answerCallbackQuery(callbackQuery.id, { 
        text: `تم اختيار ${gameTypeName} بنجاح`,
        show_alert: false 
    });
}
...
bot.onText(/\/resetconfig/, async (msg) => {
    ...
    // البحث عن اللعبة في مرحلة الإعداد
    const configGame = Object.entries(data.games).find(([gameId, game]) => 
        game.chatId === chatId && game.status === 'configuring'
    );

    if (configGame) {
        const [gameId, game] = configGame;
        
        // إعادة تعيين حالة اللعبة
        game.status = 'joining';
        game.gameType = null;
        game.waitingFor = null;
        game.configuredBy = null;
        
        await bot.sendMessage(chatId, '🔄 تم إعادة تعيين إعدادات اللعبة. يمكنكم اختيار النوع مرة أخرى.');
        
        // إعادة إرسال أزرار الاختيار
        const typeKeyboard = {
            inline_keyboard: [
                [{ text: '📦 اشياء', callback_data: `type_items_${gameId}` }],
                [{ text: '📍 اماكن', callback_data: `type_places_${gameId}` }]
        ...
        await bot.sendMessage(chatId, 'لا توجد لعبة في مرحلة الإعداد حالياً');
    }
});
// معالجة رابط الانضمام للعبة
bot.onText(/\/start join(.+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    const gameId = match[1];
    ...
    const game = data.games[gameId];
    if (!game) {
        return bot.sendMessage(chatId, 'اللعبة غير موجودة أو انتهت');
    }

    if (game.status !== 'joining') {
        return bot.sendMessage(chatId, 'اللعبة شغالة انتظر تخلص');
    }

    // فحص إذا كان اللاعب مسجل بالفعل
    if (game.players.find(p => p.id === userId)) {
        return bot.sendMessage(chatId, 'أنت مسجل بالفعل في هذه اللعبة');
    }
    ...
    // رسالة تأكيد للاعب في الخاص
    await bot.sendMessage(chatId, `✅ تم انضمامك للعبة بنجاح!
👥 عدد اللاعبين: ${game.players.length}
🎮 توجه للمجموعة لمتابعة اللعبة`);

    // تحديث قائمة اللاعبين في المجموعة
    try {
        const playersList = game.players.map((player, index) => `${index + 1}. ${player.name}`).join('\n');
        
        // إشعار انضمام في المجموعة
        await bot.sendMessage(game.chatId, `🎮 ${msg.from.first_name} انضم للعبة!

🟢 اللاعبون المنضمون:
${playersList}`);
        
    } catch (error) {
        console.log('لا يمكن إرسال إشعار للمجموعة:', error.message);
    }
});
